...
{
    if (colorForChange == MCC_YELLOW) return YELLOW;
    if (colorForChange == MCC_WHITE)  return WHITE;
    if (colorForChange == MCC_GREEN)  return GREEN;
    if (colorForChange == MCC_ORANGE) return ORANGE;
    if (colorForChange == MCC_RED)    return RED;
    if (colorForChange == MCC_BLUE)   return BLUE;
    return BLACK;
}
...
void MagicCubeGlWidget::getMagicCubeForSolve()
{
   ...
   // printf("\n");
    ...
    for (i = 0; i < temp_method_length; ++ i)
    {
        ...
        {
        case 'Y':   temp_color = S_YELLOW;  break;
        case 'W':   temp_color = S_WHITE;   break;
        case 'R':   temp_color = S_RED;     break;
        case 'G':   temp_color = S_GREEN;   break;
        case 'B':   temp_color = S_BLUE;    break;
        case 'O':   temp_color = S_ORANGE;  break;
        default:    temp_clock = S_BLACK;   break;
        }
